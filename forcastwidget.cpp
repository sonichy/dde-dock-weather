      ...
      m_settings("deepin", "dde-dock-HTYWeather")
{
    ...
    for (int i=0; i<6; i++) {
        ...
        if (i==0) {
            labelWImg[i]->setPixmap(QPixmap(":icon/NA.png").scaled(60, 60));
            ...
            labelWImg[i]->setPixmap(QPixmap(":icon/NA.png").scaled(40, 40));
...
void ForcastWidget::updateWeather()
{
    ...
    if (JPE.error == QJsonParseError::NoError) {
        if (JD.isObject()) {
            ...
            if(JO.contains("cityInfo")){
                city = JO.value("cityInfo").toObject().value("city").toString().replace("市","");
                labelDate[0]->setText(city);
                QString SUT = JO.value("cityInfo").toObject().value("updateTime").toString();
